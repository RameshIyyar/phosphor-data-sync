        ...
        convertSyncTypeToEnum(config["SyncType"]).value_or(SyncType::Immediate))
{
    ...
    {
        _retry = Retry(config["RetryAttempts"],
                       convertISODurationToSec(config["RetryInterval"])
                           .value_or(DEFAULT_RETRY_INTERVAL));
    }
    else
    {
        _retry = std::nullopt;
...
std::optional<uint16_t> DataSyncConfig::convertISODurationToSec(
